...
async fn test_streaming_persistence_basic_flow() {
    ...
    state.execute(Command::Set("key1".to_string(), sds("value1")));
    state.execute(Command::Set("key2".to_string(), sds("value2")));
    state.execute(Command::Set("key3".to_string(), sds("value3")));
...
async fn test_streaming_persistence_localfs() {
    ...
    // Execute commands
    for i in 0..10 {
        state.execute(Command::Set(format!("key{}", i), sds(&format!("value{}", i))));
    }
...
async fn test_streaming_persistence_high_throughput() {
    ...
    for i in 0..num_commands {
        state.execute(Command::Set(format!("k{}", i), sds(&format!("v{}", i))));
    }
...
async fn test_streaming_persistence_state_recovery() {
    ...
    // Phase 1: Write some data
    {
        ...
        state.execute(Command::Set("persistent_key".to_string(), sds("persistent_value")));
        state.execute(Command::Set("another_key".to_string(), sds("another_value")));
...
async fn test_streaming_persistence_multi_replica() {
    ...
    async fn run_replica(
        ...
        for (key, value) in commands {
            state.execute(Command::Set(key.to_string(), SDS::from_str(value)));
        }
...
async fn test_streaming_persistence_segment_ordering() {
    ...
    for i in 0..25 {
        state.execute(Command::Set(format!("seq{:03}", i), sds(&format!("v{}", i))));
...
async fn test_streaming_persistence_deletes() {
    ...
    // Set and then delete
    state.execute(Command::Set("to_delete".to_string(), sds("temporary")));
    state.execute(Command::Del("to_delete".to_string()));
    state.execute(Command::Set("to_keep".to_string(), sds("permanent")));
